...
// The derived auto-clip event vocabulary (`EventKind`, `EventMoment`,
// `GameEvent`) now lives in the shared games layer so every game shares one
// taxonomy. Re-exported here so existing `valorant::model::EventKind` paths
// (reconcile, summary, the cut) keep resolving.
pub use crate::games::event::{EventKind, EventMoment, GameEvent};
