...
use crate::library::db::{
    ClipRecord, CustomGame, EventMark, Library, NewClip,
};
...
// Tauri's `generate_handler!` resolves each command's generated `__cmd__*`
// macro at the literal path it is given, so these stay `pub mod` and `main.rs`
// registers them as `commands::editor::*` / `commands::storage::*`. The plain
// helpers below are re-exported so the rest of the crate keeps its old paths.
pub mod editor;
pub mod storage;

pub use storage::{allow_storage_asset_scope, storage_root};
use storage::{buffer_dir, clip_dir, clip_output_path, settings_path};
